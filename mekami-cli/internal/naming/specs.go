...
// output stable.
//
// The list is split into five conceptual groups:
//
...
works end-to-end against the local graph.`,
	...
	// ───── 5b. Core / language indexer registry ─────────────────
	{
		Use:   "core-install <lang>[@<version>]",
...
// LookupByUse returns the Spec with the given CLI Use string. The
// Use string is the canonical CLI identifier; for commands with no
// args, Use equals the command name. Returns nil if not found.
func LookupByUse(use string) *Spec {
	for i := range Specs {
		if Specs[i].Use == use {
			return &Specs[i]
		}
